...
class WideResNet(nn.Module):
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0):
        ...
        self.conv1 = nn.Conv2d(2, nChannels[0], kernel_size=3, stride=1,
                               padding=1, bias=False)
        # 1st block
    ...
    def forward(self, x, is_feat=False, preact=False, is_grad=False):
        ...
        out_grad = out
        if is_grad:
            out_grad.retain_grad()
        out = self.avgpool(out_grad)
        ...
        if is_feat:
            if is_grad:
                ...
                channel_weight = self.cal_grad(out, grad_out, [out_grad])
            ...
            if is_grad:
                return [f0, f1, f2, f3, f4], out, channel_weight
    ...
    def cal_grad(self, out, grad_out, feature):
        ...
        channel_weight = torch.sum(torch.sum(grad, 3, keepdim=True), 2, keepdim=True)
        return channel_weight

    def forward(self, x, is_feat=False, preact=False, require_grad=False):
        out = self.conv1(x)
        f0 = out
        out = self.block1(out)
        f1 = out
        ...
        if is_feat:
            if preact:
                ...
                if require_grad:
                    ind = out.data.max(1)[1]
                    grad_out = out.data.clone().fill_(0.0).scatter_(1, ind.unsqueeze(0).t(), 1.0)
                    channel_weight = self.cal_grad(out, grad_out, [pre_f3])
                return [f0, f1, f2, pre_f3, f4], out, channel_weight
            else:
                if require_grad:
                    ind = out.data.max(1)[1]
                    grad_out = out.data.clone().fill_(0.0).scatter_(1, ind.unsqueeze(0).t(), 1.0)
                    channel_weight = self.cal_grad(out, grad_out, [f3])
                return [f0, f1, f2, f3, f4], out
        else:
            return out
    '''
