...
from dataset.sound_prompt import get_detached_sound_dataloaders
...
def parse_option():
    ...
    parser.add_argument('--model', type=str, default='resnet110',
                        choices=['resnet20', 'resnet32', 'resnet56', 'resnet110', 'resnet14', 'resnet44','resnet50','resnet34',
                                 ...
                                 'vgg8','vgg11','vgg13','vgg16','vgg19','resnet101', 'MobileNetV2','simple', 'wrn_16_1','MobileNetV2_0.35','MobileNetV2_1.0',
...
def main():
    ...
    if opt.run_flag==1:
        wandb.init(
            project="sound_prediction_newsample".format(opt.dataset),
            name="Prompt-{}-{}-{}-{}-Baseline".format(opt.model,opt.optimizer,int(1000*opt.learning_rate),opt.trial),
            config={
    ...
    if opt.dataset == 'sound':
        train_loader, val_loader, n_data= get_detached_sound_dataloaders(path='./assets/data/',batch_size=opt.batch_size, num_workers= opt.num_workers)
        n_cls = 1
    else:
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if valid_rmse < best_loss:
            ...
            torch.save(model.state_dict(), './assets/model/prompt-{}-{}-{}-{}.pt'.format(opt.model,opt.optimizer,int(1000*opt.learning_rate),opt.trial))
