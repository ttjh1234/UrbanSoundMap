...
def main():
    ...
    if opt.dataset == 'sound':
        if opt.method == 'two':
            ...
            np.save('./assets/sound_map/std_vec',std_vec.to('cpu').numpy())
            n_cls = 1
            mdict = tm_dict
        elif opt.emthod == 'single':
            train_loader, mean_vec, std_vec, n_data= get_single_soundmap_dataloaders(path='./assets/newdata/',batch_size=opt.batch_size, num_workers= opt.num_workers)
            np.save('./assets/sound_map/single_mean_vec',mean_vec.to('cpu').numpy())
            np.save('./assets/sound_map/single_std_vec',std_vec.to('cpu').numpy())
            n_cls = 1
            mdict = sm_dict
        else:
