...
from dataset.sound_multiple import get_multiple_sound_dataloaders,get_multiple_two_sound_dataloaders
...
def main():
    ...
    if opt.run_flag==1:
        wandb.init(
            project="sound_multiple_prediction",
    ...
    # dataloader
    if opt.mtype == 'single':
        train_loader, val_loader, n_data= get_multiple_sound_dataloaders(path='./assets/multiple/',batch_size=opt.batch_size, num_workers= opt.num_workers,seed= opt.trial, multiple_factor = opt.multiple_factor)
        n_cls = int(opt.multiple_factor **2)   
        # model
        model = sm_dict[opt.model](num_classes=n_cls)
    elif opt.mtype == 'two':
        train_loader, val_loader, n_data= get_multiple_two_sound_dataloaders(path='./assets/multiple/',batch_size=opt.batch_size, num_workers= opt.num_workers,seed= opt.trial, multiple_factor = opt.multiple_factor)
        n_cls = int(opt.multiple_factor **2)   
        # model
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if valid_rmse < best_loss:
            ...
            torch.save(model.state_dict(), './assets/multiple_model/{}-{}-{}-{}-{}-{}.pt'.format(opt.mtype, opt.multiple_factor,opt.model,opt.optimizer,int(1000*opt.learning_rate),opt.trial))
