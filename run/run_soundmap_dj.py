...
from dataset.sound_ood import get_two_dj_sound_dataloaders
...
def main():
    ...
    if opt.run_flag==1:
        wandb.init(
            ...
            name="DJ-Two-{}-{}-{}-{}-Baseline".format(opt.model,opt.optimizer,int(1000*opt.learning_rate),opt.trial),
            config={
    ...
    if opt.dataset == 'sound':
        train_loader, mean_vec, std_vec= get_two_dj_sound_dataloaders(path='./assets/newdata/',batch_size=opt.batch_size, num_workers= opt.num_workers)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        torch.save(model.state_dict(), './assets/sound_map/dj-two-{}-{}-{}-{}.pt'.format(opt.model,opt.optimizer,int(1000*opt.learning_rate),opt.trial))
