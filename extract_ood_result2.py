...
def main():
    ...
    set_random_seed(0)
    if opt.mtype == 'single':
        mean_v, std_v = get_mean_std_single(path='./assets/newdata/',seed = 0)
        test_loader = get_ood_sound_dataloaders(path='./assets/newdata/', region = opt.region, mean_vec=mean_v, std_vec=std_v,
                                                        batch_size=opt.batch_size, num_workers=opt.num_workers)
        n_cls = 1
        mdict = sm_dict
    elif opt.mtype == 'four':
        #mean_v, std_v = get_mean_std_datached(path='./assets/newdata/',seed = 0)
        mean_v = np.array([0,0,0,0])
        std_v = np.array([1,1,1,1])
    ...
    if opt.mtype == 'single':
        model_path = './assets/aug_exp_model/'+'{}-{}-ADAM-1-{}.pt'.format('Aug_eff',mname,0)
        model = load_teacher(mname,mdict,model_path,n_cls)
    elif opt.mtype =='four':
        model_path = './assets/model/'+'no_normal-{}-{}-ADAM-1-{}.pt'.format('four',mname,0)
        model = load_teacher(mname,mdict,model_path,n_cls)
    ...
    np.save('./assets/ood_file/no_normal_pred_vgg13_{}'.format(opt.region),pred)
    np.save('./assets/ood_file/no_normal_target_vgg13_{}'.format(opt.region),target)
