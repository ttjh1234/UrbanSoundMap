...
def train_multiple(model, iterator, optimizer, criterion, device, run=0):
    ...
    for _, batch in tqdm(enumerate(iterator)):
        ...
        src = batch[0].to(device) # B,1, 100,100
        target = batch[1].to(device) # B, multiple_facor
...
def evaluate_multiple(model, iterator, criterion, device, run=0):
