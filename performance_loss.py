...
def main():
    opt = parse_option()
    model_name_list = ['four-vgg13','four-vgg16','weighted-40.0-2.0-vgg13','weighted-40.0-4.0-vgg13','weighted-50.0-2.0-vgg13','weighted-50.0-4.0-vgg13'
                       ,'weighted-40.0-2.0-vgg16','weighted-40.0-4.0-vgg16','weighted-50.0-2.0-vgg16','weighted-50.0-4.0-vgg16']
    ...
    for i in range(3):
        ...
        for mname in model_name_list:
            ...
            model_path = './assets/model/'+'{}-ADAM-1-{}.pt'.format(mname,i)
            model_bb = mname.split('-')[-1]
            model = load_teacher(model_bb,mdict,model_path,n_cls)
    ...
    for mname in model_name_list:
        if opt.metric =='rmse':
            ...
            db_group = [30,40,50,60,70,80,90,100]
            group_rmse = np.array(result_dict[mname]) 
            mean_rmse = np.round(np.mean(group_rmse,axis=0),3)
            std_rmse = np.round(np.std(group_rmse,axis=0),3)
            print("#######################################################")
            print("{}".format(mname))
            old_db = 0
            for db in db_group:
