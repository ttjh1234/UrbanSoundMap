...
def main():
    ...
    if opt.method == 'single':
        ...
        train_loader, valid_loader, ndata = get_three_sound_dataloaders(path='./assets/newdata/', resolution=opt.resolution_list[:-1], batch_size=opt.batch_size, num_workers=opt.num_workers, seed = opt.trial)
        n_cls = 1
        mdict = mm_dict
        
    elif opt.method == 'four':
        train_loader, valid_loader, ndata = get_four_sound_dataloaders(path='./assets/newdata/', resolution=opt.resolution_list[:-1], batch_size=opt.batch_size, num_workers=opt.num_workers, seed = opt.trial)
        n_cls = 1
        mdict = fm_dict
