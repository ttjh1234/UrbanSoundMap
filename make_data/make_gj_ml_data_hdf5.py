...
def make_representation_pooling(data, npool):
    # data : The number of Neighbor * 7 dim.
    hw = data.shape[0]
    h = int(np.sqrt(hw))
...
def main():
    ...
    for i in tqdm(range(coord.shape[0])):
        temp_data = make_grid(coord.iloc[i],99,100)
    ...
    hdf5_file = './assets/newdata/urbanform{}pool{}.h5py'.format(opt.resolution, opt.pool)

    with h5py.File(hdf5_file, 'w') as hf:
        hf.create_dataset('data',data=rep_var, compression = 'gzip')
