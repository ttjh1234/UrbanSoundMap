...
from dataset.sound_da2 import get_sample_ood_two_sound_dataloaders,get_sample_ood_sound_dataloaders,get_sample_ood_detached_sound_dataloaders, get_sample_ood_detached_origin_sound_dataloaders
...
def main():
    ...
    if opt.run_flag==1:
        wandb.init(
            project="sound_prediction_finetune2",
    ...
    if opt.method == 'single':
        ...
        train_loader, mean_v, std_v = get_sample_ood_two_sound_dataloaders(path='./assets/newdata/', region = opt.dataset, sample_size=opt.d_num,
                                                                           batch_size=opt.batch_size, num_workers=opt.num_workers)
        ...
        train_loader, mean_v, std_v = get_sample_ood_detached_origin_sound_dataloaders(path='./assets/newdata/', region = opt.dataset,sample_size = opt.d_num,
                                                                                       batch_size=opt.batch_size, num_workers=opt.num_workers)
        ...
        train_loader, mean_v, std_v = get_sample_ood_detached_sound_dataloaders(path='./assets/newdata/', region = opt.dataset,sample_size=opt.d_num,
                                                                                batch_size=opt.batch_size, num_workers=opt.num_workers)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        if valid_rmse < best_loss:
            ...
            torch.save(model.state_dict(), './assets/model_transfer/{}-{}-{}-{}-{}-{}-{}-{}-{}.pt'.format(opt.dataset,
                                                                                                       opt.method,
                                                                                                       opt.model,
                                                                                                       opt.batch_size,
                                                                                                       opt.n_layer,
