...
class VGG(nn.Module):
    ...
    def __init__(self, cfg, batch_norm=False, num_classes=1000):
        ...
        self.block4 = self._make_layers(cfg[4], batch_norm, cfg[3][-1]+cfg[0][-1])

        self.fuse = build_feature_connector(cfg[0][-1],cfg[0][-1])
    ...
    def forward(self, x, is_feat=False, preact=False, is_grad=False):
        ...
        f0 = x
        pre_info1 = f0[:,:,(h//2-6):(h//2+6),(h//2-6):(h//2+6)]
        fuse_info= self.fuse(pre_info1)
        x = self.pool0(x)
        x = self.block1(x)
        f1_pre = x
        x = F.relu(x)
